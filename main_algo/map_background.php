...
<head>
    ...
    #backButton {
        position: fixed;
        top: 10px; /* 與minimap頂部對齊 */
        left: 10px; /* 距離左側10px */
        z-index: 1001; /* 確保在minimap和地圖之上 */
    ...
    #minimap-container {
        width: 45%; /* 調整寬度以容納返回按鈕 */
        height: 20%;
        position: fixed;
        z-index: 1000;  /* 確保它在 map 之上 */
        background-color: white;  /* 添加背景颜色以提高可见性 */
        top: 10px; /* 與返回按鈕對齊 */
        left: 150px; /* 再往右移，為返回按鈕留出更多空間 */
        border: 1px solid #ccc; /* 添加邊框讓它更明顯 */
        padding: 10px; /* 內邊距以提供更多空間 */
    }
...
function drawMinimap(data) {
    ...
    const bars = svg.selectAll(".bar")
        ...
        .on("click", function(event, d) {
            ...
            if (selectedTime === time) {
                ...
                d3.select(this).attr("fill", colors(time)); // 還原顏色
            } else {
                selectedTime = time;
                filterRestaurantsByTime(time); // 根據選中的時間篩選餐廳
                d3.selectAll(".bar").attr("fill", d => colors(d.time)); // 還原所有條形圖顏色
    ...
    svg.selectAll(".count")
        ...
        function filterRestaurantsByTime(time) {
            ...
            if (time) {
                // 根據選定的時間篩選餐廳
                filteredList = filteredList.filter(restaurant => parseInt(restaurant.r_time_low) === parseInt(time));
            }
